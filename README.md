...
This repository contains your sudo docker Compose (pwd.yml) and the instructions/scripts to:

1. Bring up ERPNext containers,
2. Take the canonical Frappe backup (database + files) using bench --site <site> backup --with-files,
3. Back up custom app source (and fixtures),
4. Copy backup artifacts off the container to the host and offsite,
5. Restore a full site on a fresh host (order-aware and safe),
6. Run migrations, build assets, and verify the restore.
...
sudo docker compose -f $COMPOSE_FILE logs -f $BACKEND

# If you want to watch create-site service logs:
sudo docker compose -f $COMPOSE_FILE logs -f create-site
...
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "cd $BENCH_PATH && bench --site $SITE backup --with-files"

# Verify produced backups
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "ls -lh $BENCH_PATH/sites/$SITE/private/backups"
...
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "sha256sum $BENCH_PATH/sites/$SITE/private/backups/* > $BENCH_PATH/sites/$SITE/private/backups/sha256sums.txt"
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "cat $BENCH_PATH/sites/$SITE/private/backups/sha256sums.txt"
...
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "cd $BENCH_PATH && bench --site $SITE export-fixtures || true"

# Create a tarball of a custom app and copy to host (repeat for each custom app)
BACKEND_CID=$(sudo docker compose -f $COMPOSE_FILE ps -q $BACKEND)
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "cd $BENCH_PATH && tar czf /tmp/my_app.tar.gz apps/my_app"
sudo docker cp $BACKEND_CID:/tmp/my_app.tar.gz $HOST_BACKUP_DIR/my_app-$(date +%F).tar.gz
...
CONTAINER=$(sudo docker compose -f $COMPOSE_FILE ps -q $BACKEND)
mkdir -p $HOST_BACKUP_DIR/$SITE-$(date +%F_%H%M%S)
sudo docker cp ${CONTAINER}:$BENCH_PATH/sites/$SITE/private/backups/. $HOST_BACKUP_DIR/$SITE-$(date +%F_%H%M%S)/
...
sudo docker cp ~/restore/my_app.tar.gz $(sudo docker compose -f $COMPOSE_FILE ps -q $BACKEND):/tmp/
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "cd $BENCH_PATH && tar xzf /tmp/my_app.tar.gz -C . && chown -R frappe:frappe apps/my_app || true"


Option 2 — clone from Git into a host folder that will be accessible to the container:

# on VM host
cd ~/frappe_sudo docker  # folder where compose runs
...
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "chown -R frappe:frappe /home/frappe/frappe-bench/apps/my_app || true"

C. Install app dependencies & build assets
# install python dependencies per app (if requirements.txt present)
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "cd $BENCH_PATH && for app in apps/*; do if [ -f \$app/requirements.txt ]; then pip install -r \$app/requirements.txt || true; fi; done"

# build assets
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "cd $BENCH_PATH && bench build || true"

D. Copy DB and files backup into backend container
sudo docker cp ~/restore/frontend-database-YYYY.sql.gz $(sudo docker compose -f $COMPOSE_FILE ps -q $BACKEND):$BENCH_PATH/sites/$SITE/private/backups/
sudo docker cp ~/restore/files-YYYY.tar.gz $(sudo docker compose -f $COMPOSE_FILE ps -q $BACKEND):$BENCH_PATH/sites/$SITE/private/backups/

E. Run canonical restore (inside backend container)
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "
  set -e;
  cd $BENCH_PATH;
...
# list sites
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "ls -la $BENCH_PATH/sites"

# check counts for a sample doctype (example: Item)
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "cd $BENCH_PATH && bench --site $SITE console --eval \"import frappe; print(frappe.db.sql('select count(*) from `tabItem`')[0][0])\""
...
sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "cd $BENCH_PATH && bench --site $SITE install-app my_app || true && bench --site $SITE migrate"


Fix permissions:

sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "chown -R frappe:frappe $BENCH_PATH/sites"


Restart workers:

sudo docker compose -f $COMPOSE_FILE restart queue-short queue-long scheduler websocket
...
mkdir -p "$DEST"
sudo docker compose -f $COMPOSE_FILE exec backend bash -lc "cd $BENCH_PATH && bench --site $SITE backup --with-files"
CID=$(sudo docker compose -f $COMPOSE_FILE ps -q backend)
sudo docker cp ${CID}:$BENCH_PATH/sites/$SITE/private/backups/. "$DEST/"
...
Fix: sudo docker compose -f $COMPOSE_FILE exec $BACKEND bash -lc "chown -R frappe:frappe $BENCH_PATH/sites"
